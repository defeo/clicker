...
exports.createServer = function(opts) {
    ...
    */
    ...
    /*
      SAML is handled specially: upon successful login, a bearer token is
    ...
    */
    ...
    server.post({ path: '/login/saml', version: '1.0.0' }, auth.samlCallback,
		(req, res, next) => {
			...
			return res.redirect(`${opts.saml.clientUrl}?token=${token.token}`, next);
		    } else {
    ...
    */
	...
	return models.Poll.findById(req.params.pollid)
		...
		req.poll = {
		    ...
		    obj: pollObj,
		    can: function(action) {
			return this.record.can(action, req.user.level);
		    }
		}
		next();
	    })
    ...
    */
    server.get({ path: '/answer/:pollid', version: '1.0.0' },
	       ...
	       .exec()
	       .then((users) => {
		   const canRead = req.poll.can('readAnswer');
    ...
    */
    server.post({ path: '/answer/:pollid', version: '1.0.0' },
		...
		(req, res, next) => {
			...
			.then((result) => {
			    const grade = req.poll.can('readAnswer')
				  ? req.poll.record.grade(result.answers[result.answers.length].choices)
				  : null;
