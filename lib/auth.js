...
passport.use(new Basic(function(user, pass, next) {
    models.getOrCreateUser('basic:' + user, next,
			   config.userCanSubscribe.basic || config.userCanSubscribe);
}));
...
passport.use(new GH({
    ...
    return models.getOrCreateUser('github:' + profile.id, next,
				  config.userCanSubscribe.github || config.userCanSubscribe,
				  undefined,
